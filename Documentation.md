...
v9 不新增人工重标。贴边（远济 0.503 / 0.506/0.500）为 `uncommitted`：预测用带中心（夹在 0.5±0.03），不用 v6 主体（本地复跑曾把远济打成 0.622）。已提交后半区咏归式与 2/3 取较高值；岚下低尾不混合。产物仍为 `research_designer_mode_not_for_deployment`。云端无 JSON；本地复跑到新空目录后看留出 CSV 的远济（应接近 0.5）、咏归和岚下行。
